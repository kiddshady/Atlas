...
contextBridge.exposeInMainWorld('opal', {
  /* ── Atlas ── */
  atlas: {
    ultimo: () => call('atlas:ultimo'),
    escanear: () => call('atlas:escanear'),
    cancelar: () => call('atlas:cancelar'),
    escaneando: () => call('atlas:escaneando'),
    abrirCarpeta: (ruta) => call('atlas:abrir-carpeta', ruta),
    mostrarExe: (exe) => call('atlas:mostrar-exe', exe),
    abrirUrl: (url) => call('atlas:abrir-url', url),
    elegirCarpeta: () => call('atlas:elegir-carpeta'),
    /** Los eventos del escaneo: {tipo: 'inicio'|'proyecto'|'tamano'|'fin'|'error', ...}. */
    onEvento: (cb) => {
      const handler = (_e, ev) => cb(ev);
      ipcRenderer.on('atlas:evento', handler);
      return () => ipcRenderer.off('atlas:evento', handler);
    },
  },
