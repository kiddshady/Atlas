...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   levantar la app. Portado de Galena (y antes de Tessera y Quire), donde está probado de punta a punta.
   ═══════════════════════════════════════════════════════════════════════════ */
...
const RELEASES = 'https://github.com/kiddshady/Atlas/releases';
...
function soporte({ empaquetada = false, portable = false } = {}) {
  if (!empaquetada) {
    return { ok: false, motivo: 'Estás corriendo Atlas desde el código fuente. Acá no hay nada que actualizar: usá git.' };
  }
  if (portable) {
...
function iniciar(getWin, {
  ...
  /* Si nunca hacés click en "reiniciar", la actualización entra igual la próxima
     vez que cerrás Atlas. Es la parte que hace que esto sirva de verdad. */
  ...
  autoUpdater.on('update-available', (info) => fijar({
    ...
    nombre: String(info?.releaseName || '').trim() || `Atlas ${info?.version || ''}`.trim(),
    bytes: tamanoDe(info),
