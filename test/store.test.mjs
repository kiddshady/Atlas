...
const parche = await store.saveSettings({ medida: 'codigo' });
ok('guardar un parche no pisa el resto', parche.medida === 'codigo' && parche.escanearAlAbrir === store.DEFAULT_SETTINGS.escanearAlAbrir);
// Una clave nueva del código tiene que aparecer en un archivo viejo.
await store.writeJSON(store.SETTINGS_FILE, { schema: store.SCHEMA, medida: 'codigo' });
const completado = await store.loadSettings();
ok('las claves nuevas se completan solas', 'raices' in completado && completado.medida === 'codigo');
